...
def connect_db():
    conn = sqlite3.connect('../Gen/db/plyy.db')
...
def curator_like(c_uuid, u_uuid):
    ...
    try:
        ...
        cur.execute("INSERT INTO 'CURATOR_LIKE' VALUES (?, ?, ?)", (cl_uuid, u_uuid, c_uuid))
        conn.commit()
        conn.close()
        return True
...
'''
/plyy/api/unlike/${encodeURIComponent(u_uuid)}/${encodeURIComponent(c_uuid)}
/plyy/api/like/${encodeURIComponent(u_uuid)}/${encodeURIComponent(c_uuid)}
