...
def api_plyy_detail(id):
    ...
    tag_query = 'SELECT t.tag_name FROM TAG t JOIN TAG_PLYY tp ON t.tag_uuid=tp.tag_uuid WHERE tp.plyy_uuid=?'
    tags = db.get_query(tag_query,(id,))

    tracks_query = '''
                   SELECT t.track_uuid, t.track_title AS title, SUBSTR(t.track_album_img,6) AS img, t.track_album AS album,
                   t.track_artist AS artist, s.song_index, t.track_rtime
...
def api_song(id, song_index):
    song_query = '''
                 SELECT t.track_title AS title, t.track_artist as artist, SUBSTR(t.track_album_img,6) AS img,
                 t.track_album AS album, s.song_cmt AS comment, s.song_vid FROM TRACK t JOIN SONG s 
                 ON t.track_uuid=s.track_uuid WHERE s.plyy_uuid=? AND s.song_index=?
                 '''
